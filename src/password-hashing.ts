 ...
 * - iterations: Number of PBKDF2 iterations (between 1000 and 1000000). Defaults to 310000.
...
export async function hashPassword(
  ...
  const salt = options?.saltInBase64Url ? fromBase64Url(options.saltInBase64Url) :  crypto.getRandomValues(new Uint8Array(16));
  ...
  const iterations = options?.iterations || 310000; // ajustável
  ...
  return [
    iterations,
    toBase64Url(salt.buffer),
    toBase64Url(derivedBits)
  ].join(".");
}
